...
#endif
{
    ...
    private static readonly Func<T, object> _box;
    ...
    static Any()
    {
        ...
        {
            _equalsObject = Any.EmitDelegate<Func<T, object?, bool>>(
        ...
        {
            _equals = Any.EmitDelegate<Func<T, T?, bool>>(
                $"{type.Render()}_Equals",
        ...
        {
            _getHashCode = Any.EmitDelegate<Func<T, int>>(
        ...
        {
            _toString = Any.EmitDelegate<Func<T, string>>(
        ...
        {
            _format = Any.EmitDelegate<Func<T, string?, IFormatProvider?, string>>(
        ...
        {
            _getType = Any.EmitDelegate<Func<T, Type>>(
        ...
        if (type.IsByRef || type.IsByRefLike)
        {
            // instead of boxing, we can stringify and then box the string
            _box = static value => (object)Any<T>.ToString(value);
        }
        else if (type.IsValueType)
        {
            _box = Any.EmitDelegate<Func<T, object>>(
                $"{type.Render()}_Box",
                gen =>
                {
